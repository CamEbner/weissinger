...
# Open pdf to plot to
pp_Cl = PdfPages('ClPlots.Rect.pdf')
pp_cCl = PdfPages('cClPlots.Rect.pdf')
pp_CL = PdfPages('CLPlots.Rect.pdf')

# Collect integrated data
WIntData = sp.loadtxt('RectWeissinger/Sweep.Int.out')
WLDict = dict( zip( map(int, WIntData[:,0]), WIntData[:, 1]) )
PIntData = sp.loadtxt('RunPrandtl/Rectangular.Int.out', skiprows = 6)
PLDict = dict( zip( map(int, PIntData[:,0]), PIntData[:, 1]) )

# Loop over all alphas
for Alpha in range(11):
    WeissFile = 'RectWeissinger/Sweep.a%i.0.out' % int(Alpha)
    PranFile = 'RunPrandtl/Rectangular.a%i.0.Cl.out' % int(Alpha)
